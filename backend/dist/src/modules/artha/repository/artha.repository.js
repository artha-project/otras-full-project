...
let ArthaRepository = class ArthaRepository {
    ...
    async createOrUpdateProfile(data, percentile) {
    ...
    async updateTier2Progress(userId) {
        const profile = await this.findProfileByUserId(userId);
        if (!profile)
            throw new Error("Artha Profile not found");
        if (profile.tier1Progress < 100)
            throw new Error("Tier 1 must be completed first");
        return this.prisma.arthaProfile.update({
            where: { id: profile.id },
            data: { tier2Progress: 100 }
        });
    }
    async updateTier3Progress(userId) {
        const profile = await this.findProfileByUserId(userId);
        if (!profile)
            throw new Error("Artha Profile not found");
        return this.prisma.arthaProfile.update({
